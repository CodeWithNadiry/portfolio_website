export const projects = [
  {
    id: 1,
    liveLink: "https://laptop-shop-navy.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/laptop-shop",
    preview: "/projects/shop.jpg",
    title: "E-Commerce Platform",
    description:
      "A full-stack e-commerce platform where users can browse, search, and order products online. Includes an admin dashboard for managing products, users, and orders efficiently.",
    stack: "#React #NodeJS #Express #MongoDB",
  },

  {
    id: 2,
    liveLink: "https://food-order-app-contextapi.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/food_order_app_contextApi",
    preview: "/projects/food.png",
    title: "Food Order App",
    description:
      "A React food ordering app featuring a cart and checkout system, global state management with Context API, custom hooks, and validated form submissions.",
    stack: "#React #ContextAPI #JavaScript",
  },
  {
    id: 3,
    liveLink: "https://quiz-app-dun-rho.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/quiz_app",
    preview: "/projects/quiz.png",
    title: "Quiz Application",
    description:
      "An interactive quiz app built with React, offering timed questions, real-time answer validation, detailed result summaries, and full quiz reset functionality.",
    stack: "#React #JavaScript #ContextAPI",
  },
  {
    id: 4,
    liveLink: "https://place-picker-xuxk.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/place_picker",
    preview: "/projects/placepicker.png",
    title: "Place Picker",
    description:
      "A full-stack travel app that lets users manage visited and planned places, featuring geolocation-based sorting, smooth modal interactions, and efficient state handling.",
    stack: "#React #NodeJS #Express #ContextAPI",
  },
  {
    id: 5,
    liveLink: "https://projects-website-three.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/projects_website",
    preview: "/projects/projects.jpg",
    title: "Project Management Tool",
    description:
      "A project management tool designed to organize workflows, manage tasks in real time, and track deadlines with clear project-based structure.",
    stack: "#React #JavaScript #TailwindCSS",
  },
  {
    id: 6,
    liveLink: "https://foodies-next-app-ebon.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/foodies-next-app",
    preview: "/projects/nextjsfood.png",
    title: "Foodies App",
    description:
      "A Next.js food management app that allows users to view, add, edit, and delete food items with real-time UI updates and smooth interactions.",
    stack: "#NextJS #React #MongoDB #TailwindCSS",
  },
  {
    id: 7,
    liveLink: "https://events-management-bqc7.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/events_management",
    preview: "/projects/events.png",
    title: "Event Management App",
    description:
      "A MERN-based event management application allowing users to create, edit, and delete events with secure image uploads and a fully RESTful backend API.",
    stack: "#React #NodeJS #Express #MongoDB",
  },
  {
    id: 8,
    liveLink: "https://task-manager-kappa-three-33.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/Task-Manager",
    preview: "/projects/task_manager.png",
    title: "Task Manager App",
    description:
      "A MERN-based task manager allowing users to create, edit, and delete tasks with built-in timer functionality and localStorage persistence.",
    stack: "#React #LocalStorage #Context #useReducer",
  },
  {
    id: 9,
    liveLink: "https://ecommerce-react-project-eight.vercel.app/",
    githubLink: "https://github.com/CodeWithNadiry/Ecommerce-React-Project",
    preview: "/projects/e-commerce.png",
    title: "E-commerce App",
    description:
      "A React-based e-commerce app with Zustand state management, dynamic routing, role-based access control, and localStorage persistence.",
    stack: "#React #LocalStorage #Zustand",
  },
];
